...
export default function BlogEdit({ postId }) {
  ...
  const [videoUrl, setVideoUrl] = useState(""); // New state for video URL input
  ...
  useEffect(() => {
    ...
    authFetch(`${API}/api/blog/admin/blogs/${postId}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  const handleFeaturedImageUpload = (assets) => {
    if (assets && assets.length > 0) {
      ...
      setFeaturedImage(null); // Clear featured image when removed
    }
  };
  ...
  const handleAddVideoByUrl = () => {
    ...
    // Create video object compatible with existing structure
    const videoAsset = {
      url: videoUrl.trim(),
      resourceType: "video",
      publicId: `external_${Date.now()}`, // Generate unique ID for external videos
      format: "url", // Indicate this is an external URL
    };

    // Add to existing videos array
    setVideos((prev) => [...(prev || []), videoAsset]);
  ...
  const handleSave = async (publish = false) => {
    ...
    try {
      ...
      const method = "PUT";
      const resp = await authFetch(`${API}/api/blog/admin/blogs/${postId}`, {
        method,
    ...
    <div className="bg-white p-6 rounded shadow max-w-5xl mx-auto">
      ...
      <div className="mb-6 p-4 border rounded-lg bg-gray-50">
        ...
        <div className="mt-4 p-3 border border-gray-200 rounded-lg bg-white">
          ...
          <p className="text-xs text-gray-500 mt-1">
            Enter a direct video URL (MP4, WebM, etc.) or video platform embed
            URL
          </p>
        </div>
      </div>
